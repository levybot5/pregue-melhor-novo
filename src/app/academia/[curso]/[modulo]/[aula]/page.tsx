...
function lessonHref(curso: string, target: AcademyLesson | null): string | null {
  if (!target) return null;
  return `/academia/${curso}/${String(target.moduleId).padStart(2, "0")}/${String(
    target.lessonInModule,
  ).padStart(2, "0")}`;
}
...
export default async function AcademiaLessonPage({
  ...
  params: Promise<{ curso: string; modulo: string; aula: string }>;
}) {
  const { curso, modulo, aula } = await params;
  if (curso !== ACADEMY_COURSE.id) {
    notFound();
  }

  const moduleId = Number(modulo);
  const lessonInModule = Number(aula);
  const mod = getModule(moduleId);
  const lesson = getLesson(moduleId, lessonInModule);
  if (!mod || !lesson) {
    notFound();
  }
  ...
  if (!user) {
    redirect(`/entrar?redirectTo=/academia/${curso}/${modulo}/${aula}`);
  }

  const hasProAccess = await isSubscriptionActive(user.id);

  // Sem assinatura ativa: prévia da Academia termina aqui — mostra CTA
  // do Pro em vez do player, sem carregar nenhum iframe do YouTube.
  if (!hasProAccess) {
    return (
      <main className="mx-auto flex w-full max-w-xl flex-1 flex-col gap-6 px-4 pb-10 pt-[calc(env(safe-area-inset-top)+1.5rem)]">
        <BackLink href={`/academia/${curso}/${modulo}`} label="Voltar para o módulo" />

        <header className="flex flex-col gap-1">
          <p className="text-sm font-semibold text-muted">
            Módulo {String(mod.id).padStart(2, "0")} — {mod.title}
          </p>
        ...
        <div className="flex flex-col items-center gap-3 rounded-2xl border border-card-border bg-card p-6 text-center">
          <p className="text-foreground">
            Essa aula faz parte do Pregue Melhor Pro. Assine para assistir a todo o Curso Básico
            de Teologia.
          </p>
          <Link
            href="/planos"
  ...
  // Só registra "assistindo agora" quando o acesso é liberado — nunca
  // conta como progresso para quem só está vendo a prévia.
  await recordLessonWatched(user.id, ACADEMY_COURSE.id, moduleId, lesson.lessonNumber);
  ...
  const { previous, next } = getAdjacentLessons(lesson.lessonNumber);
  const previousHref = lessonHref(curso, previous);
  const nextHref = lessonHref(curso, next);
    ...
    <main className="mx-auto flex w-full max-w-xl flex-1 flex-col gap-5 px-4 pb-10 pt-[calc(env(safe-area-inset-top)+1.5rem)]">
      ...
      <header className="flex flex-col gap-1">
        ...
        <p className="text-xs text-muted">
          Aula {String(lesson.lessonInModule).padStart(2, "0")} de {mod.lessonCount}
        </p>
        <h1 className="text-xl font-bold leading-snug tracking-tight text-foreground">
          {lesson.title}
        </h1>
      ...
      <div className="aspect-video w-full overflow-hidden rounded-2xl bg-black">
        <iframe
          ...
          src={getLessonEmbedUrl(lesson.lessonNumber)}
          title={lesson.title}
      ...
      <div className="flex flex-wrap items-center justify-between gap-2 text-xs text-muted">
        <span>Conteúdo original de {ACADEMY_COURSE.channel} no YouTube</span>
        <a
          href={getLessonWatchUrl(lesson.lessonNumber)}
          target="_blank"
          rel="noreferrer"
